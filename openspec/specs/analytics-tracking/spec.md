...
Client-side usage analytics for the PBL Persona Test — loading Google Tag Manager and emitting a defined set of namespaced `dataLayer` events across the quiz journey, so the tool's reach and the paths through it can be measured.

Synced from `add-gtm-analytics`. Implemented in container `GTM-5LTFPDLV` against GA4 measurement ID `G-KSK91X79BX`; all eight events verified reaching GA4 on 2026-09-07.
